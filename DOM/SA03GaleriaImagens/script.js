...
function inciarCarrossel() {
  ...
  setInterval(() => {
    ...
    divCarrossel.style.transition = `transform ${
      (1 + imagensUrl.length) / imagensUrl.length
    }s ease-in-out`; //troca as imagens
    divCarrossel.style.transform = `translateX(-${
      index * (100 / imagensUrl.length)
    }%)`; //move corretamente
  }, 2000);
}
